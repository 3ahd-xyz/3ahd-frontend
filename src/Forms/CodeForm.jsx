...
export default function CodeForm() {
  ...
  const verify = async () => {
    ...
    if (inputValue.length !== 6)
      return setError(
        "الرقم المدخل لا يساوي 6 ارقام رجاءا قم بادخال رقم ملائم"
      );
    setLoading(true);

    try {
      const response = await axios.get(
        "https://api.3ahd.xyz/auth/login/" + phoneNumber + "?otp=" + inputValue
      );
      if (response?.status == 220) {
        ...
        try {
          const response = await axios.get(
            "https://api.3ahd.xyz/userBusId?phoneNumber=" +
              phoneNumber +
              "&otpCode=" +
              inputValue
          );
          ...
          if (response.data == null) {
            navigate(
              "/buses?phoneNumber=" + phoneNumber + "&otp=" + inputValue
            );
          } else {
